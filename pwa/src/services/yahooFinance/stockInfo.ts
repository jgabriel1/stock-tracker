...
const parseResponse = (
  response: AxiosResponse<YahooResponseData>,
): Map<string, YahooStock> => {
  const { data } = response
  const results = data.spark.result

  // This mapping "enters" the json object simplifying the final result
  const stocksInfo = results.map(
    (result: YahooStockResult) => result.response[0].meta,
  )
...
export const getSingleStockInfo = async (
  ticker: string,
): Promise<YahooStock> => {
